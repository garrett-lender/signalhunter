...
static void fd_to_path(int fd, char *out, size_t out_len) {
    ...
    if (rw_read_link_path(proc_path, out, out_len) != 0) snprintf(out, out_len, "<unknown>");
}
    ...
    while (1) {
        ...
        for (md = (struct fanotify_event_metadata *)buf;
            ...
            if ((md->mask & FAN_OPEN_EXEC) || rw_suspicious_path(path)) {
                printf("\n[FILE] pid=%d mask=0x%llx path=%s\n", md->pid, (unsigned long long)md->mask, path);
                rw_log_event("file pid=%d mask=0x%llx path=%s", md->pid, (unsigned long long)md->mask, path);
                if (rw_suspicious_path(path)) {
                    printf("  Reason: suspicious file path touched\n");
                    rw_log_alert("suspicious file path touched pid=%d path=%s mask=0x%llx", md->pid, path, (unsigned long long)md->mask);
                    rw_score_add(md->pid, NULL, NULL, cfg, "FILE", "suspicious file path touched", 65);
                    if (rw_case_has(md->pid)) rw_case_event(md->pid, 65, cfg, "FILE_DETAIL", "path=%s mask=0x%llx", path, (unsigned long long)md->mask);
