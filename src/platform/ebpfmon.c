...
static rw_ebpf_connect_track_t *get_or_create_connect_track(int pid, const char *comm, const char *ip, unsigned int port) {
    for (size_t i = 0; i < g_connect_track_count; i++) {
        rw_ebpf_connect_track_t *t = &g_connect_tracks[i];
        if (t->pid == pid && t->port == port && strcmp(t->ip, ip) == 0) return t;
    }
    if (g_connect_track_count >= RW_EBPF_CONNECT_TRACKS) return NULL;
...
static double ebpf_avg_interval(const rw_ebpf_connect_track_t *t) {
    if (!t || t->interval_count == 0) return 0.0;
    double sum = 0.0;
    for (size_t i = 0; i < t->interval_count; i++) sum += t->intervals[i];
    return sum / (double)t->interval_count;
}

static double ebpf_interval_jitter(const rw_ebpf_connect_track_t *t) {
    if (!t || t->interval_count < 2) return 999999.0;
...
static int ebpf_score_connect_track(const rw_ebpf_connect_track_t *t) {
    int score = 0;
    if (!t) return 0;
    if (t->total_seen >= 4) score += 25;
    if (t->interval_count >= 3) {
        double avg = ebpf_avg_interval(t);
        double jitter = ebpf_interval_jitter(t);
        if (avg >= 1.0 && avg <= 600.0 && jitter <= avg * 0.30) score += 60;
    }
    if (t->port == 80 || t->port == 443 || t->port == 8080 || t->port == 8443) score += 5;
    if (score > 100) score = 100;
    return score;
}

static void update_ebpf_connect_track(int pid, const char *comm, const char *ip, unsigned int port, const rw_config_t *cfg) {
    ...
    if (now > t->last_seen) {
        ...
        if (interval >= 1.0) {
            if (t->interval_count < 16) t->intervals[t->interval_count++] = interval;
            else {
    ...
    if (score > 0) {
        char reason[256];
        snprintf(reason, sizeof(reason), "dst=%s:%u seen=%zu avg=%.2f jitter=%.2f", t->ip, t->port, t->total_seen, ebpf_avg_interval(t), ebpf_interval_jitter(t));
    ...
    if (score >= alert_threshold && !t->already_alerted) {
        t->already_alerted = 1;
        rw_log_alert("eBPF possible beacon pid=%d comm=%s dst=%s:%u seen=%zu avg=%.2f jitter=%.2f score=%d",
                     t->pid, t->comm, t->ip, t->port, t->total_seen, ebpf_avg_interval(t), ebpf_interval_jitter(t), score);
        if (rw_case_has(t->pid)) rw_case_event(t->pid, score, cfg, "EBPF_BEACON_DETAIL", "comm=%s dst=%s:%u seen=%zu avg=%.2f jitter=%.2f",
                      t->comm, t->ip, t->port, t->total_seen, ebpf_avg_interval(t), ebpf_interval_jitter(t));
...
static const char *syscall_kind_name(unsigned int kind) {
    switch (kind) {
        case RW_BPF_KIND_PTRACE: return "ptrace";
        case RW_BPF_KIND_PVM_WRITE: return "process_vm_writev";
        case RW_BPF_KIND_PVM_READ: return "process_vm_readv";
        case RW_BPF_KIND_MPROTECT_EXEC: return "mprotect(PROT_EXEC)";
        case RW_BPF_KIND_MMAP_EXEC: return "mmap(PROT_EXEC)";
        case RW_BPF_KIND_MEMFD_CREATE: return "memfd_create";
        case RW_BPF_KIND_EXECVE: return "execve";
        case RW_BPF_KIND_EXECVEAT: return "execveat";
        case RW_BPF_KIND_CHMOD: return "chmod";
        case RW_BPF_KIND_FCHMODAT: return "fchmodat";
        case RW_BPF_KIND_UNLINK: return "unlink";
        case RW_BPF_KIND_UNLINKAT: return "unlinkat";
        default: return "unknown";
    }
}

static int bpf_path_is_suspicious(const char *path) {
    return rw_suspicious_path(path) ||
...
static void handle_socket_event(const struct rw_bpf_event *ev, const rw_config_t *cfg) {
    rw_log_alert("eBPF raw/packet socket syscall pid=%u tgid=%u comm=%s family=%s(%u) type=%u protocol=%u",
                 ev->pid, ev->tgid, ev->comm, family_name(ev->family), ev->family, ev->sock_type, ev->protocol);
    rw_score_add((int)ev->tgid, ev->comm, NULL, cfg, "EBPF_RAW_SOCKET", family_name(ev->family), 95);
    if (!cfg || cfg->log_raw) {
    ...
    printf("  Type:     %u\n", ev->sock_type);
    printf("  Protocol: %u\n", ev->protocol);
    if (rw_case_has((int)ev->tgid)) rw_case_event((int)ev->tgid, 95, cfg, "EBPF_RAW_SOCKET_DETAIL", "comm=%s family=%s(%u) type=%u protocol=%u", ev->comm, family_name(ev->family), ev->family, ev->sock_type, ev->protocol);
}
...
static void handle_connect_event(const struct rw_bpf_event *ev, const rw_config_t *cfg) {
    ...
    if (cfg && cfg->log_network) rw_log_network("ebpf_connect pid=%u tgid=%u comm=%s dst=%s:%u", ev->pid, ev->tgid, ev->comm, ip, ev->port);
    if (cfg && cfg->verbose) printf("\n[eBPF] connect pid=%u tgid=%u comm=%s dst=%s:%u\n", ev->pid, ev->tgid, ev->comm, ip, ev->port);
...
static void handle_syscall_event(const struct rw_bpf_event *ev, const rw_config_t *cfg) {
    ...
    switch (ev->kind) {
        ...
        case RW_BPF_KIND_PVM_READ:
            snprintf(detail, sizeof(detail), "target_pid=%llu local_iov=%llu remote_iov=%llu", ev->a0, ev->a1, ev->a2);
            break;
        case RW_BPF_KIND_MPROTECT_EXEC:
            snprintf(detail, sizeof(detail), "addr=0x%llx len=%llu prot=0x%llx", ev->a0, ev->a1, ev->a2);
            break;
        case RW_BPF_KIND_MMAP_EXEC:
            snprintf(detail, sizeof(detail), "addr=0x%llx len=%llu prot=0x%llx flags=0x%llx fd=%lld", ev->a0, ev->a1, ev->a2, ev->a3, (long long)ev->protocol);
            break;
        case RW_BPF_KIND_MEMFD_CREATE:
            snprintf(detail, sizeof(detail), "name=%s flags=0x%llx", ev->str[0] ? ev->str : "<unknown>", ev->a0);
            break;
        case RW_BPF_KIND_EXECVE:
        case RW_BPF_KIND_EXECVEAT:
            snprintf(detail, sizeof(detail), "path=%s flags=0x%llx", ev->str[0] ? ev->str : "<unknown>", ev->a0);
            alert = bpf_path_is_suspicious(ev->str);
            if (!alert && cfg && cfg->verbose && cfg->log_syscalls) rw_log_event("exec pid=%u comm=%s path=%s", ev->tgid, ev->comm, ev->str);
            break;
        case RW_BPF_KIND_CHMOD:
        case RW_BPF_KIND_FCHMODAT:
            snprintf(detail, sizeof(detail), "path=%s mode=0%llo", ev->str[0] ? ev->str : "<unknown>", ev->a0);
            alert = bpf_path_is_suspicious(ev->str) || ((ev->a0 & 0111) != 0 && rw_suspicious_path(ev->str));
            break;
    ...
    if (alert) rw_syscall_alert((int)ev->tgid, ev->comm, kind, detail, cfg);
}

static int handle_bpf_event(void *ctx, void *data, size_t len) {
    const rw_config_t *cfg = ctx ? (const rw_config_t *)ctx : g_runtime_cfg;
    if (len < sizeof(struct rw_bpf_event)) return 0;
    const struct rw_bpf_event *ev = data;

    g_events_seen++;
    if (ev->event_type == RW_BPF_EVENT_SOCKET) { g_socket_seen++; handle_socket_event(ev, cfg); }
    else if (ev->event_type == RW_BPF_EVENT_CONNECT) { g_connect_seen++; handle_connect_event(ev, cfg); }
    else if (ev->event_type == RW_BPF_EVENT_SYSCALL) { g_syscall_seen++; handle_syscall_event(ev, cfg); }
    return 0;
}
...
int rw_ebpf_init_backend(void) {
    ...
    if (!g_obj) { errno = ENOENT; return -1; }
    ...
    bpf_object__for_each_program(prog, g_obj) {
        if (g_link_count >= sizeof(g_links) / sizeof(g_links[0])) break;
        struct bpf_link *link = bpf_program__attach(prog);
        err = libbpf_get_error(link);
        if (err) {
            rw_log_info("eBPF attach failed for program %s: %ld; continuing", bpf_program__name(prog), err);
            continue;
        }
        if (link) g_links[g_link_count++] = link;
    }
    ...
    if (!map) { rw_ebpf_shutdown(); errno = ENOENT; return -1; }
    ...
    if (!g_rb) { rw_ebpf_shutdown(); return -1; }
...
void rw_ebpf_poll(int fd, const rw_config_t *cfg) {
    ...
    if (g_rb) {
        ...
        if (rc < 0 && rc != -EINTR) fprintf(stderr, "[warn] eBPF ring poll failed: %d\n", rc);
    }
}
...
void rw_ebpf_shutdown(void) {
    ...
    for (size_t i = 0; i < g_link_count; i++) {
        if (g_links[i]) bpf_link__destroy(g_links[i]);
        g_links[i] = NULL;
    }
    g_link_count = 0;
    if (g_obj) { bpf_object__close(g_obj); g_obj = NULL; }
}

#else
int rw_ebpf_init_backend(void) { errno = ENOSYS; return -1; }
void rw_ebpf_poll(int fd, const rw_config_t *cfg) { (void)fd; (void)cfg; }
void rw_ebpf_shutdown(void) {}
void rw_ebpf_print_status(void) { printf("[ebpf] status: not compiled in; rebuild with make ebpf\n"); }
void rw_ebpf_selftest_hint(void) { printf("eBPF is not compiled in; rebuild with make ebpf first.\n"); }
#endif
