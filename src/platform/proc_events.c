            ...
            switch (ev->what)
            {
                case PROC_EVENT_EXEC:
                    ...
                    printf("\n[PROC] exec pid=%d tgid=%d\n", ev->event_data.exec.process_pid,
                           ev->event_data.exec.process_tgid);
                    rw_log_event("proc exec pid=%d tgid=%d", ev->event_data.exec.process_pid,
                                 ev->event_data.exec.process_tgid);
                ...
                case PROC_EVENT_FORK:
                    ...
                    if (cfg->verbose)
                    {
                        printf("\n[PROC] fork parent=%d child=%d\n", ev->event_data.fork.parent_pid,
                               ev->event_data.fork.child_pid);
                        rw_log_event("proc fork parent=%d child=%d", ev->event_data.fork.parent_pid,
                                     ev->event_data.fork.child_pid);
                    }
                    break;
                case PROC_EVENT_EXIT:
                    ...
                    if (cfg->verbose)
                    {
                        ...
                        rw_log_event("proc exit pid=%d code=%u", ev->event_data.exit.process_pid,
                                     ev->event_data.exit.exit_code);
                    }
                    break;
                case PROC_EVENT_UID:
