...
size_t rw_collect_inode_owners(rw_inode_owner_t *owners, size_t max_owners) {
    ...
    while ((de = readdir(proc)) != NULL && count < max_owners) {
        ...
        while ((fd_de = readdir(fds)) != NULL && count < max_owners) {
            ...
            if (npath < 0 || (size_t)npath >= sizeof(fd_path)) continue;
...
static int proc_tcp4_hex_to_str(const char *hex, char *out, size_t out_len) {
    ...
    unsigned char b[4];
    if (hex_byte(hex + 6, &b[0]) || hex_byte(hex + 4, &b[1]) ||
        hex_byte(hex + 2, &b[2]) || hex_byte(hex + 0, &b[3])) return -1;
...
static int proc_tcp6_hex_to_str(const char *hex, char *out, size_t out_len) {
    ...
    for (int word = 0; word < 4; word++) {
        ...
        if (hex_byte(p + 6, &b[word * 4 + 0]) ||
            hex_byte(p + 4, &b[word * 4 + 1]) ||
            hex_byte(p + 2, &b[word * 4 + 2]) ||
            hex_byte(p + 0, &b[word * 4 + 3])) return -1;
    }
...
static int is_browser_exe(const char *exe) {
    if (!exe) return 0;
    return strstr(exe, "/firefox") || strstr(exe, "/chrome") ||
           strstr(exe, "/chromium") || strstr(exe, "/brave") ||
           strstr(exe, "/msedge") || strstr(exe, "/opera") ||
...
static size_t collect_tcp_file(const char *path, int family,
                               const rw_inode_owner_t *owners, size_t owner_count,
    ...
    while (fgets(line, sizeof(line), fp) && count < max_events) {
        ...
        int matched = sscanf(line,
            " %*d: %79[0-9A-Fa-f]:%X %79[0-9A-Fa-f]:%X %X %*s %*s %*s %*s %*s %lu",
            local_addr, &local_port, remote_addr, &remote_port, &state, &inode);

        if (matched != 6) continue;
        if (state != TCP_ESTABLISHED && state != TCP_SYN_SENT) continue;

        rw_inode_owner_t owner;
        if (!find_owner_by_inode(owners, owner_count, inode, &owner)) continue;

        char dst_ip[RW_MAX_IP_STR];
        int rc = (family == AF_INET)
            ? proc_tcp4_hex_to_str(remote_addr, dst_ip, sizeof(dst_ip))
            : proc_tcp6_hex_to_str(remote_addr, dst_ip, sizeof(dst_ip));
        if (rc != 0) continue;

        if (strcmp(dst_ip, "0.0.0.0") == 0 || strcmp(dst_ip, "::") == 0) continue;
...
size_t rw_collect_tcp_events(const rw_inode_owner_t *owners, size_t owner_count,
                             rw_conn_event_t *events, size_t max_events) {
    size_t count = 0;
    count += collect_tcp_file("/proc/net/tcp", AF_INET, owners, owner_count,
                              events + count, max_events - count);
    if (count < max_events) {
        count += collect_tcp_file("/proc/net/tcp6", AF_INET6, owners, owner_count,
                                  events + count, max_events - count);
    }
    return count;
}
...
static int raw_alert_seen(int pid, unsigned long inode) {
    for (size_t i = 0; i < g_raw_alert_count; i++) {
        if (g_raw_alerts[i].pid == pid && g_raw_alerts[i].inode == inode) return 1;
    }
...
static void scan_raw_file(const char *path, const char *kind,
                          const rw_inode_owner_t *owners, size_t owner_count,
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        rw_inode_owner_t owner;
        if (!find_owner_by_inode(owners, owner_count, inode, &owner)) continue;
        if (raw_alert_seen(owner.pid, inode)) continue;

        rw_log_alert("raw socket detected kind=%s pid=%d exe=%s inode=%lu", kind, owner.pid, owner.exe, inode);
        rw_score_add(owner.pid, NULL, owner.exe, cfg, "RAW_SOCKET", kind, 95);
        if (rw_case_has(owner.pid)) rw_case_event(owner.pid, 95, cfg, "RAW_SOCKET_DETAIL", "kind=%s exe=%s inode=%lu", kind, owner.exe, inode);
        if (!cfg || cfg->log_raw) rw_log_network("raw_socket kind=%s pid=%d exe=%s inode=%lu", kind, owner.pid, owner.exe, inode);
        ...
        if (cfg && cfg->stop_suspicious) {
            ...
            if (rc != 0) rw_log_alert("SIGSTOP failed pid=%d errno=%d", owner.pid, errno);
        }
    }
    fclose(fp);
}
...
static void scan_packet_file(const rw_inode_owner_t *owners, size_t owner_count,
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        rw_score_add(owner.pid, NULL, owner.exe, cfg, "PACKET_SOCKET", "packet/raw capture socket", 95);
        if (rw_case_has(owner.pid)) rw_case_event(owner.pid, 95, cfg, "PACKET_SOCKET_DETAIL", "exe=%s inode=%lu", owner.exe, inode);
        if (!cfg || cfg->log_raw) rw_log_network("packet_socket pid=%d exe=%s inode=%lu", owner.pid, owner.exe, inode);
...
static rw_conn_track_t *get_or_create_track(const rw_conn_event_t *ev) {
    for (size_t i = 0; i < g_track_count; i++) {
        ...
        if (t->pid == ev->pid && t->dst_port == ev->dst_port && strcmp(t->dst_ip, ev->dst_ip) == 0) return t;
    }
    if (g_track_count >= RW_MAX_TRACKS) return NULL;
...
static rw_syn_track_t *get_or_create_syn_track(int pid, const char *exe, time_t now) {
    for (size_t i = 0; i < g_syn_track_count; i++) {
        if (g_syn_tracks[i].pid == pid) return &g_syn_tracks[i];
    }
    if (g_syn_track_count >= RW_SYN_TRACKS) return NULL;
...
static void update_syn_scan_track(const rw_conn_event_t *ev, const rw_config_t *cfg) {
    ...
    for (size_t i = 0; i < t->unique_count; i++) {
        if (strcmp(t->keys[i], key) == 0) { known = 1; break; }
    }
    ...
    if (!t->already_alerted && t->unique_count >= (size_t)threshold_unique && t->syn_count >= (size_t)threshold_syn) {
        t->already_alerted = 1;
        rw_log_alert("possible synscan pid=%d exe=%s syn_sent=%zu unique_targets=%zu window=30s", t->pid, t->exe, t->syn_count, t->unique_count);
        rw_score_add(t->pid, NULL, t->exe, cfg, "SYNSCAN", "many SYN_SENT connections to unique targets", 90);
        if (rw_case_has(t->pid)) rw_case_event(t->pid, 90, cfg, "SYNSCAN_DETAIL", "exe=%s syn_sent=%zu unique_targets=%zu window=30s", t->exe, t->syn_count, t->unique_count);
        ...
        if (cfg && cfg->stop_suspicious) {
            ...
            if (rc != 0) rw_log_alert("SIGSTOP failed pid=%d errno=%d", t->pid, errno);
        }
    }
}

static double avg_interval(const rw_conn_track_t *t) {
    if (t->interval_count == 0) return 0.0;
    double sum = 0.0;
...
static double interval_jitter(const rw_conn_track_t *t) {
    if (t->interval_count < 2) return 999999.0;
...
static int score_track(const rw_conn_track_t *t) {
    ...
    if (t->total_seen >= 4) score += browser ? 5 : 25;

    if (t->interval_count >= 3) {
        double avg = avg_interval(t);
        double jitter = interval_jitter(t);
        if (avg >= 2.0 && avg <= 600.0 && jitter <= avg * 0.30) score += browser ? 10 : 60;
    }

    if (is_stable_browser_port(t->dst_port)) score += browser ? 0 : 5;
    if (rw_suspicious_path(t->exe)) score += 30;
     ...
     * executable path. */
    if (browser && !rw_suspicious_path(t->exe) && score < 85) return 0;
...
void rw_update_track(const rw_conn_event_t *ev, const rw_config_t *cfg) {
    if (ev->state == TCP_SYN_SENT) {
        if (cfg && cfg->log_synscan) rw_log_network("syn_sent pid=%d exe=%s dst=%s:%u", ev->pid, ev->exe, ev->dst_ip, ev->dst_port);
    ...
    if (cfg && cfg->log_network) {
        rw_log_network("tcp_established pid=%d exe=%s dst=%s:%u seen=%zu", ev->pid, ev->exe, ev->dst_ip, ev->dst_port, t->total_seen);
    }
    if (cfg && cfg->verbose) {
        printf("[NET] pid=%d exe=%s dst=%s:%u seen=%zu\n", ev->pid, ev->exe, ev->dst_ip, ev->dst_port, t->total_seen);
    }
    ...
    if (score > 0) {
        char reason[256];
        snprintf(reason, sizeof(reason), "dst=%s:%u seen=%zu avg=%.2f jitter=%.2f", t->dst_ip, t->dst_port, t->total_seen, avg_interval(t), interval_jitter(t));
    ...
    if (score >= alert_threshold && !t->already_alerted) {
        t->already_alerted = 1;
        rw_log_alert("possible beaconing pid=%d exe=%s dst=%s:%u seen=%zu avg=%.2f jitter=%.2f score=%d", t->pid, t->exe, t->dst_ip, t->dst_port, t->total_seen, avg_interval(t), interval_jitter(t), score);
        if (rw_case_has(t->pid)) rw_case_event(t->pid, score, cfg, "BEACON_DETAIL", "exe=%s dst=%s:%u seen=%zu avg=%.2f jitter=%.2f", t->exe, t->dst_ip, t->dst_port, t->total_seen, avg_interval(t), interval_jitter(t));
        ...
        if (cfg->stop_suspicious) {
            ...
            if (rc != 0) { perror("kill SIGSTOP"); rw_log_alert("SIGSTOP failed pid=%d", t->pid); }
        } else {
