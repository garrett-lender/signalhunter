...
int rw_case_has(int pid) { return find_case(pid) != NULL; }

static void write_proc_file_to_case(int pid, const char *name,
                                    const char *dst_name)
{
...
static void snapshot_case(int pid, const rw_config_t *cfg)
{
  ...
  if (c)
  {
    ...
    (void)rw_score_write_case(pid, c->dir);
...
static rw_case_t *create_case(int pid, int score, const rw_config_t *cfg,
                              const char *reason)
{
  ...
  const char *base =
      (cfg && cfg->log_dir && *cfg->log_dir) ? cfg->log_dir : "logs";
  ...
  if (n >= 0 && (size_t)n < sizeof(path))
  {
    ...
    if (fp)
    {
      ...
      fprintf(fp, "[%s] [CASE_OPEN] pid=%d score=%d exe=%s reason=%s\n",
              human_ts, pid, score, exe, reason ? reason : "<none>");
      fclose(fp);
    }
  }

  rw_log_alert("case opened pid=%d score=%d dir=%s reason=%s", pid, score,
               c->dir, reason ? reason : "<none>");
  printf("\n[case] opened %s\n", c->dir);
  snapshot_case(pid, cfg);
  return c;
}

static rw_case_t *get_or_create_case(int pid, int score, const rw_config_t *cfg,
                                     const char *reason)
{
  rw_case_t *c = find_case(pid);
  if (c)
...
static void case_vlog(rw_case_t *c, int score, const char *category,
                      const char *fmt, va_list ap)
{
  ...
  fprintf(fp, "[%s] [%s] score=%d %s\n", ts, category ? category : "EVENT",
          score, msg);
  fclose(fp);
}

void rw_case_event(int pid, int score, const rw_config_t *cfg,
...
void rw_case_action(int pid, const rw_config_t *cfg, const char *action,
