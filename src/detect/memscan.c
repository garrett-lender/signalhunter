...
static int interesting_mapping(const sh_map_region_t* r)
{
    ...
    if (has_x && strstr(r->path, "(deleted)"))
    {
        return 1;
    }

    if (has_x && strstr(r->path, "/memfd:"))
    {
        return 1;
    }

    if (has_x && rw_suspicious_path(r->path))
    {
        return 1;
    }
...
static int parse_maps_line(const char* line, sh_map_region_t* r)
{
    ...
    if (sscanf(range, "%lx-%lx", &r->start, &r->end) != 2)
    {
        return -1;
    }
    ...
    if (n == 3)
    {
        snprintf(r->path, sizeof(r->path), "%s", map_path);
    }
...
static int has_pe_header(const uint8_t* buf, size_t len)
{
    ...
    if ((size_t)peoff + 4 > len)
    {
        return 0;
    }

    return buf[peoff] == 'P' && buf[peoff + 1] == 'E' && buf[peoff + 2] == 0 &&
           buf[peoff + 3] == 0;
}
...
int sh_memscan_pid(const sh_memscan_request_t* req)
{
    ...
    {
        sh_map_region_t r;
        if (parse_maps_line(line, &r) != 0)
        {
            continue;
        }

        if (!interesting_mapping(&r))
        {
            continue;
        }
        ...
        if (cap == 0 || cap > SH_MEMSCAN_MAX_REGION_DUMP)
        {
            cap = SH_MEMSCAN_MAX_REGION_DUMP;
        }
        ...
        if (nread <= 0)
        {
            fprintf(findings, "read failed region=%lx-%lx perms=%s path=%s err=%s\n", r.start,
                    r.end, r.perms, r.path, strerror(errno));
            free(buf);
            continue;
        }

        fprintf(findings, "\nregion=%lx-%lx read=%d/%zu perms=%s path=%s\n", r.start, r.end, nread,
                to_read, r.perms, r.path);
        ...
        if (has_elf_header(buf, (size_t)nread))
        {
            ...
            rw_score_add(req->pid, NULL, NULL, cfg, "memory", "ELF header in suspicious mapping",
                         60);
            findings_count++;
        }
        ...
        if (has_pe_header(buf, (size_t)nread))
        {
            ...
            rw_score_add(req->pid, NULL, NULL, cfg, "memory", "PE header in suspicious mapping",
                         60);
            findings_count++;
        }
        ...
        if (strstr(r.path, "/memfd:"))
        {
            ...
            rw_score_add(req->pid, NULL, NULL, cfg, "memory", "executable memfd mapping scanned",
                         35);
            findings_count++;
        }
        ...
        if (sc.finding_count > 0)
        {
            ...
            rw_score_add(req->pid, NULL, NULL, cfg, "memory",
                         "shellcode-like byte patterns in suspicious mapping",
                         sc.score >= 60 ? 60 : sc.score);
