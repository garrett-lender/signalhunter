...
int rw_scan_injection_pid(int pid, int verbose, const rw_config_t* cfg)
{
    ...
    if (!fp)
    {
        if (verbose)
        {
            printf("[scan] PID %d maps unavailable: %s\n", pid, strerror(errno));
        }
    ...
    while (fgets(line, sizeof(line), fp))
    {
        ...
        else if (has_x && !has_path)
        {
            reason = NULL;
        }
        else if (has_x && strstr(map_path, "(deleted)"))
        {
            reason = "deleted executable mapping";
        }
        else if (has_x && strstr(map_path, "/memfd:"))
        {
            reason = "executable memfd mapping";
        }
        else if (has_x && rw_suspicious_path(map_path))
        {
            reason = "executable mapping from suspicious path";
        }
        ...
        if (reason)
        {
            ...
            if (rw_case_has(pid))
                rw_case_event(pid, 80, cfg, "INJECTION_DETAIL", "exe=%s reason=%s map=%s", exe,
                              reason, line);
            {
                printf("\n[INJECTION?] PID %d\n", pid);
            }
    ...
    if (fp)
    {
        while (fgets(line, sizeof(line), fp))
        {
            ...
            {
                findings++;
                rw_log_injection("pid=%d exe=%s reason=process is being ptraced tracer=%d", pid,
                                 exe, tracer);
                rw_score_add(pid, NULL, exe, cfg, "INJECTION", "process is being ptraced", 90);

                if (rw_case_has(pid))
                    rw_case_event(pid, 90, cfg, "INJECTION_DETAIL",
                                  "exe=%s reason=process is being ptraced tracer=%d", exe, tracer);
                {
                    printf("\n[INJECTION?] PID %d\n", pid);
                }
