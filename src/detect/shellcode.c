...
static void add_finding(sh_shellcode_report_t *r, int points, const char *msg)
{
    ...
    if (r->finding_count < SH_SHELLCODE_MAX_FINDINGS) {
        snprintf(r->findings[r->finding_count], sizeof(r->findings[r->finding_count]), "%s", msg);
        r->finding_count++;
    }
    r->score += points;
...
{
    if (!buf || !pat || pat_len == 0 || len < pat_len) return 0;
...
static int repeated_pattern(const uint8_t *buf, size_t len, const uint8_t *pat, size_t pat_len, size_t repeat)
{
    if (!buf || !pat || pat_len == 0 || repeat == 0 || len < pat_len * repeat) return 0;
...
static void scan_x86(const uint8_t *buf, size_t len, sh_shellcode_report_t *r)
{
    ...
    if (has_run_byte(buf, len, 0x90, 16)) add_finding(r, 25, "x86/x64 NOP sled: >=16 consecutive 0x90 bytes");
    if (has_run_byte(buf, len, 0x90, 32)) add_finding(r, 20, "x86/x64 long NOP sled: >=32 consecutive 0x90 bytes");
    if (has_pattern(buf, len, syscall_pat, sizeof(syscall_pat))) add_finding(r, 25, "x86_64 syscall instruction pattern: 0f 05");
    if (has_pattern(buf, len, int80_pat, sizeof(int80_pat))) add_finding(r, 25, "x86 int 0x80 syscall pattern: cd 80");
    if (has_pattern(buf, len, binsh, sizeof(binsh) - 1)) add_finding(r, 20, "shell string present: /bin/sh");
    if (has_pattern(buf, len, mov_rax_syscall, sizeof(mov_rax_syscall))) add_finding(r, 10, "x64 syscall setup pattern: mov rax, imm32");
}
...
static void scan_arm64(const uint8_t *buf, size_t len, sh_shellcode_report_t *r)
{
    ...
    if (repeated_pattern(buf, len, nop_le, sizeof(nop_le), 8) || repeated_pattern(buf, len, nop_be, sizeof(nop_be), 8))
        add_finding(r, 25, "ARM64 NOP sled: repeated NOP instructions");
    if (has_pattern(buf, len, svc_le, sizeof(svc_le)) || has_pattern(buf, len, svc_be, sizeof(svc_be)))
...
static void scan_arm32_thumb(const uint8_t *buf, size_t len, sh_shellcode_report_t *r)
{
    ...
    if (repeated_pattern(buf, len, arm_nop_le, sizeof(arm_nop_le), 8) || repeated_pattern(buf, len, arm_nop_be, sizeof(arm_nop_be), 8))
        add_finding(r, 20, "ARM NOP sled: repeated ARM NOP instructions");
    if (repeated_pattern(buf, len, thumb_nop_le, sizeof(thumb_nop_le), 12) || repeated_pattern(buf, len, thumb_nop_be, sizeof(thumb_nop_be), 12))
        add_finding(r, 20, "Thumb NOP sled: repeated Thumb NOP instructions");
    if (has_pattern(buf, len, arm_svc_le, sizeof(arm_svc_le)) || has_pattern(buf, len, arm_svc_be, sizeof(arm_svc_be)))
...
static void scan_mips(const uint8_t *buf, size_t len, sh_shellcode_report_t *r)
{
    ...
    for (size_t i = 0; i + 3 < len; i += 4) {
        ...
        if (le == 0 || be == 0) {
            zero_words++;
            if (zero_words > max_zero_words) max_zero_words = zero_words;
        } else {
            zero_words = 0;
        }

        if (mips_syscall_word(le) || mips_syscall_word(be)) saw_syscall = 1;
    }

    if (max_zero_words >= 8) add_finding(r, 20, "MIPS NOP sled: repeated zero-word NOPs");
    if (saw_syscall) add_finding(r, 20, "MIPS syscall instruction pattern");
}
...
void sh_shellcode_write_report(FILE *fp, const sh_shellcode_report_t *report)
{
    ...
    for (int i = 0; i < report->finding_count; i++) fprintf(fp, "shellcode_indicator=%s\n", report->findings[i]);
}
