...
void rw_monitor_all(const rw_config_t *cfg) {
    if (cfg->log_max_bytes > 0) { char b[64]; snprintf(b, sizeof(b), "%ld", cfg->log_max_bytes); setenv("RW_LOG_MAX_BYTES", b, 1); }
    if (cfg->log_archives >= 0) { char b[64]; snprintf(b, sizeof(b), "%d", cfg->log_archives); setenv("RW_LOG_ARCHIVES", b, 1); }
    if (cfg->log_rate_seconds >= 0) { char b[64]; snprintf(b, sizeof(b), "%d", cfg->log_rate_seconds); setenv("RW_LOG_RATE_SECONDS", b, 1); }
    ...
    printf("Logs: %s/{signalhunter,alerts,network,injection,events}.log%s\n", cfg->log_dir, cfg->use_syslog ? " and syslog/journald" : "");
    printf("Case files: %s/cases/ when score >= %d\n", cfg->log_dir, cfg->case_threshold);
    rw_log_info("daemon started poll=%d action=%s log_network=%s", cfg->poll_seconds, cfg->stop_suspicious ? "stop" : "alert-only", cfg->log_network ? "yes" : "no");
    ...
    while (1) {
        ...
        for (size_t i = 0; i < event_count; i++) rw_update_track(&events[i], cfg);
        ...
        if (cfg->injection_scan_seconds > 0 && now - last_full_injection_scan >= cfg->injection_scan_seconds) {
...
void rw_usage(const char *argv0) {
    ...
    printf("  --quiet-raw            do not write raw socket detections to network.log; alerts still fire\n");
    printf("  --quiet-syn            do not write SYN_SENT observations to network.log; alerts still fire\n");
    printf("  --quiet-syscalls       do not write suspicious syscall observations to events.log; alerts still fire\n");
    printf("  --syslog               also write to syslog/journald; view with journalctl -t signalhunter\n");
    printf("  --case-threshold <n>   create per-process case file at score n, default: 60\n");
    printf("  --injection-scan-seconds <n> full maps/injection scan interval, default: 30; 0 disables\n");
...
static void parse_options(int argc, char **argv, int start, rw_config_t *cfg) {
    for (int i = start; i < argc; i++) {
        if (strcmp(argv[i], "--no-proc-events") == 0) cfg->enable_proc_events = 0;
        else if (strcmp(argv[i], "--no-fanotify") == 0) cfg->enable_fanotify = 0;
        else if (strcmp(argv[i], "--no-ebpf") == 0) cfg->enable_ebpf = 0;
        else if (strcmp(argv[i], "--stop-suspicious") == 0) cfg->stop_suspicious = 1;
        else if (strcmp(argv[i], "--verbose") == 0) cfg->verbose = 1;
        else if (strcmp(argv[i], "--quiet-network") == 0) cfg->log_network = 0;
        else if (strcmp(argv[i], "--quiet-raw") == 0) cfg->log_raw = 0;
            ...
            if (i + 1 < argc) {
                ...
                if (cfg->case_threshold < 1) cfg->case_threshold = 60;
                if (cfg->case_threshold > 100) cfg->case_threshold = 100;
            } else fprintf(stderr, "[warn] --case-threshold requires a number\n");
        }

        else if (strcmp(argv[i], "--injection-scan-seconds") == 0) {
            if (i + 1 < argc) { cfg->injection_scan_seconds = atoi(argv[++i]); if (cfg->injection_scan_seconds < 0) cfg->injection_scan_seconds = 0; }
            else fprintf(stderr, "[warn] --injection-scan-seconds requires a number\n");
        }
        else if (strcmp(argv[i], "--log-dir") == 0) {
            if (i + 1 < argc) cfg->log_dir = argv[++i];
            else fprintf(stderr, "[warn] --log-dir requires a directory\n");
        }
        else if (strcmp(argv[i], "--log-max-bytes") == 0) {
            if (i + 1 < argc) { cfg->log_max_bytes = atol(argv[++i]); if (cfg->log_max_bytes < 0) cfg->log_max_bytes = 0; }
            else fprintf(stderr, "[warn] --log-max-bytes requires a number\n");
        }
        else if (strcmp(argv[i], "--log-archives") == 0) {
            if (i + 1 < argc) { cfg->log_archives = atoi(argv[++i]); if (cfg->log_archives < 0) cfg->log_archives = 0; }
            else fprintf(stderr, "[warn] --log-archives requires a number\n");
        }
...
int main(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "monitor") == 0) {
        ...
        if (argc >= 3 && rw_is_numeric(argv[2])) {
            ...
            if (cfg.poll_seconds <= 0) cfg.poll_seconds = 2;
            opt_start = 3;
        }
