...
static int open_log_file(rw_log_sink_t *sink, const char *dir, const char *name, const char *level, int pri) {
    int n = snprintf(sink->path, sizeof(sink->path), "%s/%s", dir, name);
    if (n < 0 || (size_t)n >= sizeof(sink->path)) return -1;
...
static void rotate_if_needed(rw_log_sink_t *sink) {
    if (!sink || !sink->fp || g_log_max_bytes <= 0) return;
    long pos = ftell(sink->fp);
    if (pos >= 0 && pos < g_log_max_bytes) return;
...
int rw_log_init(const char *log_dir, int use_syslog) {
    ...
    if (n > 0 && (size_t)n < sizeof(cases_dir)) ensure_dir(cases_dir);

    if (open_log_file(&g_log_main, dir, "signalhunter.log", "INFO", LOG_INFO) != 0) return -1;
    if (open_log_file(&g_log_alerts, dir, "alerts.log", "ALERT", LOG_WARNING) != 0) return -1;
    if (open_log_file(&g_log_network, dir, "network.log", "NET", LOG_INFO) != 0) return -1;
    if (open_log_file(&g_log_injection, dir, "injection.log", "INJECTION", LOG_WARNING) != 0) return -1;
    if (open_log_file(&g_log_events, dir, "events.log", "EVENT", LOG_INFO) != 0) return -1;
    if (open_log_file(&g_log_scores, dir, "scores.log", "SCORE", LOG_INFO) != 0) return -1;
    ...
    if (max_env && *max_env) g_log_max_bytes = atol(max_env);
    if (archives_env && *archives_env) g_log_archives = atoi(archives_env);
    if (rate_env && *rate_env) g_log_rate_seconds = atoi(rate_env);
    if (g_log_max_bytes < 0) g_log_max_bytes = RW_DEFAULT_LOG_MAX_BYTES;
    if (g_log_archives < 0) g_log_archives = RW_DEFAULT_LOG_ARCHIVES;
    if (g_log_rate_seconds < 0) g_log_rate_seconds = RW_DEFAULT_LOG_RATE_SECONDS;

    g_use_syslog = use_syslog;
    if (g_use_syslog) openlog("signalhunter", LOG_PID | LOG_NDELAY, LOG_DAEMON);

    rw_log_info("logger initialized dir=%s syslog=%s max_bytes=%ld archives=%d rate_seconds=%d", dir, use_syslog ? "yes" : "no", g_log_max_bytes, g_log_archives, g_log_rate_seconds);
    return 0;
}

void rw_log_close(void) {
    rw_log_sink_t *sinks[] = { &g_log_main, &g_log_alerts, &g_log_network, &g_log_injection, &g_log_events, &g_log_scores };
    for (size_t i = 0; i < sizeof(sinks)/sizeof(sinks[0]); i++) {
        if (sinks[i]->fp) fclose(sinks[i]->fp);
        sinks[i]->fp = NULL;
    }
...
static int should_emit_line(const char *level, const char *msg, unsigned int *suppressed_out) {
    if (g_log_rate_seconds == 0) return 1;
    char key[4608];
...
static void write_line(rw_log_sink_t *sink, const char *level, const char *msg, unsigned int suppressed) {
    ...
    if (suppressed > 0) fprintf(sink->fp, " suppressed_repeats=%u", suppressed);
    fputc('\n', sink->fp);
    fflush(sink->fp);
}

static void vlog_to(rw_log_sink_t *sink, const char *level, int syslog_pri, const char *fmt, va_list ap) {
    char msg[4096];
    vsnprintf(msg, sizeof(msg), fmt, ap);

    unsigned int suppressed = 0;
    if (!should_emit_line(level, msg, &suppressed)) return;

    write_line(sink, level, msg, suppressed);
    if (&g_log_main != sink) write_line(&g_log_main, level, msg, 0);
    if (g_use_syslog) syslog(syslog_pri, "%s", msg);
}
...
void rw_syscall_alert(int pid, const char *comm, const char *kind, const char *detail, const rw_config_t *cfg) {
    ...
    rw_log_alert("suspicious syscall pid=%d comm=%s kind=%s detail=%s", pid, safe_comm, safe_kind, safe_detail);
    if (!cfg || cfg->log_syscalls) {
        rw_log_event("suspicious_syscall pid=%d comm=%s kind=%s detail=%s", pid, safe_comm, safe_kind, safe_detail);
    }
    ...
    printf("  COMM:   %s\n", safe_comm);
    printf("  Type:   %s\n", safe_kind);
    if (safe_detail[0]) printf("  Detail: %s\n", safe_detail);

    int score = 75;
    if (strstr(safe_kind, "ptrace") || strstr(safe_kind, "process_vm_writev") || strstr(safe_kind, "mprotect") || strstr(safe_kind, "mmap")) score = 85;
    rw_score_add(pid, safe_comm, NULL, cfg, "SYSCALL", safe_kind, score);
    if (rw_case_has(pid)) rw_case_event(pid, score, cfg, "SYSCALL_DETAIL", "comm=%s kind=%s detail=%s", safe_comm, safe_kind, safe_detail);
}
